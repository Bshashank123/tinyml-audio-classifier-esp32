...
void setupWebServer() {
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
        ...
        // Note: Content-Encoding gzip would be added here if the placeholder was actually gzipped
    ...
    server.on("/classify", HTTP_POST, [](AsyncWebServerRequest *request){
        // Request finished receiving body
        ...
        // 1. Receive body into static buffer
        if (index == 0) {
            memset(audio_buffer, 0, sizeof(audio_buffer));
        }
        
        // Data contains raw int16 PCM (little endian expected)
        size_t offset = index / 2;
        size_t samples = len / 2;
        if (offset + samples <= 16000) {
